// copied from https://github.com/codemirror/closebrackets/blob/main/src/closebrackets.ts
...
export function insertBracket(state: EditorState, bracket: string): Transaction | null {
  ...
  for (let tok of tokens) {
    ...
    if (bracket == tok) {
      return closed == tok ? handleSame(state, tok) 
...
// Handles cases where the open and close token are the same.
function handleSame(state: EditorState, token: string) {
  let dont = null, changes = state.changeByRange(range => {
    if (!range.empty)
