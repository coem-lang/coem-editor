...
})({"224n6":[function(require,module,exports) {
"use strict";
...
const navToggle = document.querySelector(".navToggle"), nav = document.querySelector(".nav"), menuHeaderBtns = document.querySelectorAll(".nav__item > button"), menuBtns = document.querySelectorAll(".nav__dropdown-item > button"), filenameTitle = document.querySelector(".filename-title"), exampleBtns = document.querySelectorAll(".example-item button"), regex = document.querySelector(".regex");
...
function onMenuHeaderMouseover(e) {
    ...
    let btn = e.currentTarget;
    if (isMenuOpen) {
        for (let b of menuHeaderBtns)b.parentElement.classList.remove("nav__item--open");
...
function load(filename) {
    fetch("./examples/" + filename + ".coem").then((r)=>r.text()).then((code)=>{
        setDoc(code);
...
// CHARACTER INPUT
function insert(str) {
    (0, _setupJs.view).dispatch((0, _setupJs.view).state.replaceSelection(str));
    (0, _setupJs.view).focus();
}
function setDoc(str) {
    (0, _setupJs.view).dispatch({
        changes: {
            from: 0,
            to: (0, _setupJs.view).state.doc.length,
            insert: str
        }
    });
    (0, _setupJs.view).focus();
...
document.addEventListener("DOMContentLoaded", ()=>{
    ...
    navToggle.addEventListener("click", (e)=>{
        e.preventDefault();
        if (nav.classList.toggle("open")) navToggle.innerText = "\xd7";
...
document.addEventListener("keydown", (e)=>{
    if (e.code === "Escape" && regex.classList.contains("visible")) regex.classList.remove("visible");
});

},{"./codemirror/setup.js":"2qpg6"}]},["224n6","goJYj"], "goJYj", "parcelRequire34c6")
